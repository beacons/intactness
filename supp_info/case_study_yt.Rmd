...
mining <- shapefile(paste0("../data/",region,"/vector/mining.shp"))
mining <- spTransform(mining, CRS("+proj=longlat +datum=WGS84"))
...
m <- leaflet(bnd, width=900, height=600) %>%
	...
	addPolygons(data=mining, fill=F, weight=2, color="darkblue", fillOpacity=1, group="Placer mining") %>%
	...
	addLayersControl(position = "topright",
		...
		overlayGroups = c("Ecoregion","Placer mining","CIFL2013","GIFL2013","HF2009","FF","UNUSED","WILD","MSA"),
		options = layersControlOptions(collapsed = FALSE)) %>%
		hideGroup(c("Placer mining","CIFL2013","GIFL2013","HF2009","FF","UNUSED","WILD","MSA"))
...
```{r echo=FALSE, message=F, warning=F}
x <- read.csv(paste0("../data/",region,"/placer_streams_accuracy.csv"), as.is=T)
x$map <- toupper(x$map)
names(x) <- c("Intactness Map","Study area (km2)","Intact area (km2)","Intact area (%)","Mining area (km2)","Area omitted (km2)","Area omitted (%)")
kable(x, align=c('l','c','c','c','c','c','c'),caption='**Table 1**. Placer streams 500m buffer area omitted by each intactness map in the Yukon study area.')
```

&nbsp;

```{r echo=FALSE, message=F, warning=F}
x <- read.csv(paste0("../data/",region,"/placer_claims_accuracy.csv"), as.is=T)
x$map <- toupper(x$map)
names(x) <- c("Intactness Map","Study area (km2)","Intact area (km2)","Intact area (%)","Mining area (km2)","Area omitted (km2)","Area omitted (%)")
kable(x, align=c('l','c','c','c','c','c','c'),caption='**Table 2**. Place claims area omitted by each intactness map in the Yukon study area.')
