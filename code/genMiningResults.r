...
r_mining = shapefile(paste0("data/",region,"/vector/mining.shp"))
...
for (i in maps) {
	...
	d <- read.dbf(paste0("data/",region,"/vector/",i,"_mining.dbf"))
	if (nrow(d)==0) {
		x[x$map==i, "omitted_km2"] <- 0
	} else {
		v <- shapefile(paste0("data/",region,"/vector/",i,"_mining.shp"))
...
write_csv(x, paste0("data/",region,"/placer_mining_accuracy.csv"))
